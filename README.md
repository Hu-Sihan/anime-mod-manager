...
| 本地模组扫描、搜索、筛选、启用 / 禁用和批量操作 | 元数据分发 CDN 与更快的远端模组加载链路 |
| 每个模组目录内的 `.anime-mod.json` 元数据持久化 | 通过脚本实现模组合并与游戏内热切换 |
| `config.json` 配置文件，以及下载并发、语言标识、夜间模式等设置项保存 | 着色器地址映射修复 |
| GIMI runtime 检查、下载和更新，默认来源为 `SilentNightSound/GIMI-Package` | Linux 端跨 Wine/WinSurf 环境的模组注入 |
| `zip` / `7z` / `rar` 压缩包安装支持 | UI 交互和设置页内容还需要继续完善 |

### 🗺️ 未来开发计划

- [ ] 完善 UI 交互和设置页内容，让常用配置、运行时管理和下载管理入口更清晰。
- [ ] 建立元数据分发 CDN，降低对本地缓存的依赖，提高远端模组列表和详情加载速率。
