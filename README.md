...
Anime Mod Manager 是一个面向 GIMI / 3dmigoto 模组工作流的桌面模组管理器。目前项目主要围绕 **Genshin Impact** 的 GameBanana 模组浏览、下载、安装和本地管理展开，主要面向想在一个窗口里浏览、下载、安装和启停 GIMI 模组的普通用户。

当前仓库更接近“可运行的桌面 demo + 核心功能模块”，还不是带安装器、自动更新和多平台打包的正式发行版。
...
| ✅ 已实现 | ❌ 还没实现 / 待完善 |
| --- | --- |
| Genshin Impact 的 GameBanana 模组浏览、详情查看、下载和安装 | Windows 构建与面向普通用户的安装包 |
| 下载队列、并发控制、暂停 / 继续、失败状态记录和重启恢复 | 多款游戏的前端选择、下载和本地管理流程 |
| 本地模组扫描、搜索、筛选、启用 / 禁用和批量操作 | 通过脚本实现模组合并与游戏内热切换 |
| 每个模组目录内的 `.anime-mod.json` 元数据持久化 | 着色器地址映射修复 |
| GIMI runtime 检查、下载和更新，默认来源为 `SilentNightSound/GIMI-Package` | Linux 端跨 Wine/WinSurf 环境的模组注入 |
| `zip` / `7z` / `rar` 压缩包安装支持 | 多语言覆盖更多 UI 组件 |
| 暗色模式（亮 / 暗切换，CSS 变量适配） | |
| 多语言支持（zh-CN / en-US / ja-JP），即时切换 | |
| 元数据分发 CDN（Cloudflare Worker + R2 + KV），增量同步 | |
| CDN 加速地址配置与连通性测试 | |

### 🗺️ 未来开发计划

- [x] ~~建立元数据分发 CDN~~ — Cloudflare Worker + R2 + KV，每小时增量同步，详情懒加载。
- [x] ~~完善 UI 交互和设置页内容~~ — 暗色模式、多语言即时切换、CDN 配置与测试。
- [ ] 实现 Windows 构建，补齐面向普通用户的桌面运行环境。
- [ ] 实现多款游戏的模组下载与管理，不再只面向单一游戏前端流程。
- [ ] 通过脚本实现模组合并，支持游戏内热切换等更灵活的使用方式。
- [ ] 修复着色器地址映射相关问题，提高模组注入和运行稳定性。
- [ ] 实现 Linux 端跨 Wine/WinSurf 环境的模组注入能力。
...
- 🗂️ **本地元数据按需读取**：`MetaManager` 只维护 `uuid -> 目录 / 模板类型` 索引，避免启动时一次性读入全部模组详情。
- 🧩 **启用 / 禁用不破坏目录结构**：本地模组通过启用目录和禁用目录切换状态，保留模组自身文件和管理器元数据。
- 🖼️ **图片与封面缓存**：在线浏览和本地详情会复用已缓存的图片资源，减少重复请求。
- ⚙️ **运行时管理入口**：设置页提供 GIMI runtime 安装、版本检查和更新能力。
- 🛠️ **核心库与 UI 分离**：下载、安装、GameBanana API 和 meta 管理在核心库中，GTK 前端只是其中一个客户端。
- ☁️ **元数据 CDN 加速**：可通过 Cloudflare Worker 分发模组列表和详情，支持增量同步、懒加载详情，不可用时自动回退直连。
- 🌙 **暗色模式**：亮 / 暗主题切换，CSS 全面适配 libadwaita 变量，标签、卡片、抽屉均正确渲染。
- 🌐 **多语言支持**：简体中文、English、日本語，设置页即时切换。
...
- 新增 UI 功能时，优先让核心能力留在 `src/`，避免把业务逻辑写死在 `demo/src/ui/`。
